...
import { AccountCircle, People, ManageAccounts, Logout, Warning, ExpandMore, Search } from '@mui/icons-material';
...
const AdminDashboard = () => {
  ...
  // Component for Complaint Card - simplified
  const ComplaintCard = ({ complaint }) => (
    <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)', borderRadius: '12px' }}>
      <CardContent sx={{ flexGrow: 1 }}>
        ...
        {/* Action buttons */}
        <Box sx={{ marginTop: 2, display: 'flex', justifyContent: 'space-between', gap: 1 }}>
          {[
    ...
    <Box display="flex" minHeight="100vh" bgcolor="#f5f5f5">
      ...
      >
        ...
        <Box p={2} textAlign="center" sx={{ bgcolor: '#2c3e50' }}>
          <Box display="flex" justifyContent="center" alignItems="center" mb={1}>
            <Image src="/images/sklogo3.png" alt="Logo" width={60} height={60} />
            <Typography 
              variant="h4" 
              sx={{ 
                ml: 1, 
          ...
          <Typography variant="body2" sx={{ color: '#ccc', fontSize: '12px', mt: -1 }}>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', flexGrow: 1, overflow: 'auto' }}>
          {renderMenuItem(<AccountCircle />, "Complaints", "complaints", <ExpandMore />)}
          
          {/* Complaints submenu */}
          <Box 
            sx={{ 
              maxHeight: expandedMenu ? '500px' : '0px',
              overflow: 'hidden',
      ...
      >
        ...
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <Image 
            src="/images/cityseal1.png" // Replace with actual city logo path
      ...
      <Box component="main" sx={{ 
        ...
        p: 3, 
        ml: '240px',
        mt: '64px', // Added top margin to account for header
        backgroundColor: '#f5f5f5',
        minHeight: 'calc(100vh - 64px)'
      }}>
        {/* Title based on current view */}
        <Typography variant="h5" fontWeight="bold" color="#3a86a8" mb={3}>
          {({
      ...
      {/* Notify Dialog */}
      <Dialog open={openNotifyDialog} onClose={() => setOpenNotifyDialog(false)}>
